...
- **Ghost insertion selection**: Inserting a ghost atom now preserves the current atom selection instead of automatically selecting the newly created ghost.
...
- **Compact ORCA Cartesian constraints**: Consecutive fixed atom indices are now serialized as ORCA ranges such as `{ C 2:5 C }`, including axis-specific Cartesian constraints.
...
- **Constraint export cleanup**: POSCAR, QE, ABACUS STRU, and native `.acoord` exports now omit fixed-atom or selective-dynamics flags when all atoms are unconstrained, while preserving real full or partial movement constraints.
...
- **Selected atom fractional coordinates**: The Properties panel now displays read-only fractional coordinates next to Cartesian coordinates when the selected atom belongs to a structure with a unit cell.
...
- **OpenMX fixed constraints after atom deletion**: Saving an edited `.dat` file after deleting atoms now writes exactly one updated `MD.Fixed.XYZ` block with current atom indices.

## [0.3.15]

### Fixed

- **OpenMX species preservation**: Saving edited `.dat` files now updates `Species.Number` and `Definition.of.Atomic.Species` from the current atom species while preserving existing species rows, adding defaults for new species, and removing species no longer present.
