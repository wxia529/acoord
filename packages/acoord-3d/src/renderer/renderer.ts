...
export class StructureRenderer {
  ...
  init(canvas: HTMLCanvasElement): void {
    ...
    this.handlers.setStatus('Canvas size: ' + Math.round(width) + 'x' + Math.round(height));
    ...
    try {
      ...
      this.handlers.setError('WebGL renderer failed to initialize. Check GPU/WebGL support.');
      return;
    }

    const gl = rendererState.renderer!.getContext();
    if (!gl) {
      this.handlers.setError('WebGL context unavailable. Your system or VS Code may have WebGL disabled.');
      return;
    }
  ...
  renderStructure(data: Structure, hooks?: Partial<UiHooks>, options?: { fitCamera?: boolean }): void {
...
function updateLightsForCamera(): void {
  ...
  _keyOffset.set(getLightingStore().keyLight?.x ?? 0, getLightingStore().keyLight?.y ?? 0, getLightingStore().keyLight?.z ?? 10);
  _fillOffset.set(getLightingStore().fillLight?.x ?? -10, getLightingStore().fillLight?.y ?? -5, getLightingStore().fillLight?.z ?? 5);
  _rimOffset.set(getLightingStore().rimLight?.x ?? 0, getLightingStore().rimLight?.y ?? 5, getLightingStore().rimLight?.z ?? -10);
...
function buildUnitCellGroup(edges: UnitCellEdge[], scale: number): THREE.Group | null {
  ...
  const thickness = Number.isFinite(getDisplayStore().unitCellThickness)
    ? Math.max(0.5, Math.min(6, getDisplayStore().unitCellThickness)) : 1;
...
function fitCamera(): void {
  ...
  if (rendererState.camera instanceof THREE.OrthographicCamera) {
    ...
    const cameraDistance = Math.abs(maxDim / 2 / Math.tan(fov / 2)) * 0.6 / getDisplayStore().viewZoom;
...
function updateLighting(): void {
  ...
  rendererState.ambientLight.intensity = enabled ? (getLightingStore().ambientIntensity ?? 0.5) * Math.PI : 0;
  rendererState.keyLight.intensity = enabled ? (getLightingStore().keyLight?.intensity ?? 0.7) * Math.PI : 0;
  rendererState.fillLight.intensity = enabled ? (getLightingStore().fillLight?.intensity ?? 0) * Math.PI : 0;
  rendererState.rimLight.intensity = enabled ? (getLightingStore().rimLight?.intensity ?? 0) * Math.PI : 0;
  rendererState.ambientLight.color.set(resolveLightColor(getLightingStore().ambientColor, '#ffffff'));
  rendererState.keyLight.color.set(resolveLightColor(getLightingStore().keyLight?.color, '#CCCCCC'));
  rendererState.fillLight.color.set(resolveLightColor(getLightingStore().fillLight?.color, '#ffffff'));
  rendererState.rimLight.color.set(resolveLightColor(getLightingStore().rimLight?.color, '#ffffff'));
